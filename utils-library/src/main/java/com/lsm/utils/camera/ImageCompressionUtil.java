...
public class ImageCompressionUtil {
    private static ImageCompressionUtil instance = null;
    private Context context;
    private CompressListener compressListener;
    public static final int DEFAULT_OUTWIDTH = 500;
    public static final int DEFAULT_OUTHEIGHT = 500;
    private static final int DEFAULT_MAXFILESIZE = 1024;//KB

    private ImageCompressionUtil(Context context) {
        this.context = context;
    }

    public static ImageCompressionUtil getInstance(Context context) {
        if (instance == null) {
            synchronized (ImageCompressionUtil.class) {
                if (instance == null)
                    instance = new ImageCompressionUtil(context.getApplicationContext());
            }
        }
        return instance;
    }

    public ImageCompressionUtil withListener(CompressListener compressListener) {
    ...
    public String compressImage(String srcImageUri, int outWidth, int outHeight, int maxFileSize) {
        ...
        if (outHeight != 0 && outWidth != 0){
            ...
            actualOutWidth = maxWidth;
            float srcRatio = maxWidth / srcWidth;
            actualOutHeight = srcRatio * srcHeight;
        }
        options.inSampleSize = computSampleSize(options, actualOutWidth, actualOutWidth);
        ...
        int options_ = 100;
        actualOutBitmap.compress(format, options_, baos);//质量压缩方法，把压缩后的数据存放到baos中 (100表示不压缩，0表示压缩到最小)
        ...
        while (baosLength / 1024 > maxFileSize) {//循环判断如果压缩后图片是否大于maxMemmorrySize,大于继续压缩
            ...
            actualOutBitmap.compress(format, options_, baos);//将压缩后的图片保存到baos中
            baosLength = baos.toByteArray().length;
            if (options_ <= min)//如果图片的质量已降到最低则，不再进行压缩
                break;
        }
    ...
    private String getOutputFileName(String srcFilePath) {
        ...
        File file = new File(Environment.getExternalStorageDirectory().getPath(), "ImageCompressionUtil/Images");
        if (!file.exists()) {
            file.mkdirs();
        }
    ...
    public void starCompress(String srcImageUri, int outWidth, int outHeight, int maxFileSize) {
        new CompressTask().execute(srcImageUri, String.valueOf(outWidth), String.valueOf(outHeight), String.valueOf(maxFileSize));
    }

    public void starCompressWithDefault(String srcImageUri) {
        new CompressTask().execute(srcImageUri, String.valueOf(DEFAULT_OUTWIDTH), String.valueOf(DEFAULT_OUTHEIGHT), String.valueOf(DEFAULT_MAXFILESIZE));
    }
    ...
    public interface CompressListener {
        ...
        /**
         * 压缩结束
         * @param imageOutPath 可在该回调方法中进行如下逻辑获得压缩后的文件
         *
         *  if (imageOutPath.getStatus() == ImageCompressionUtil.CompressResult.RESULT_ERROR)//压缩失败
         *  return;
